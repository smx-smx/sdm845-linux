...
enum fg_sram_param_id {
	...
	FG_SETTING_ADC_CONF,
	FG_SETTING_RS_TO_RSLOW_CHG,
	FG_SETTING_RS_TO_RSLOW_DISCHG,
	FG_SETTING_PROFILE_INTEGRITY_REG,
	FG_SETTING_ALG,
...
out:
	rc = fg_set_sram_param(chip, FG_DATA_VINT_ERR, buf, 0);
	if (rc < 0)
...
static int fg_learn_cap(struct fg_chip *chip)
{
	...
	rc = fg_get_param(chip, FG_DATA_ACT_CAP,
			&chip->learning_data.actual_cap_uah, false);
	if (rc)
	...
	if (!chip->learning_data.init_cc_pc_val) {
		rc = fg_get_param(chip, FG_DATA_CHARGE, &temp, false);
		if (rc) {
...
static int fg_hw_init_pmi8950(struct fg_chip *chip)
{
	...
	if (chip->param[FG_SETTING_THERMAL_COEFFS].length > 0) {
		rc = fg_set_sram_param(chip, FG_SETTING_THERMAL_COEFFS,
				chip->batt_info.thermal_coeffs, false);
		if (rc) {
...
static int get_vbat_est_diff(struct fg_chip *chip, int *diff)
{
	...
	rc = fg_get_param(chip, FG_DATA_VOLTAGE, &volt, false);
	if (rc) {
		pr_err("failed to read voltage\n");
		return rc;
	}

	rc = fg_get_param(chip, FG_DATA_CPRED_VOLTAGE, &pred_volt, false);
	if (rc) {
...
static int fg_update_batt_rslow_settings(struct fg_chip *chip)
{
	...
	rc = fg_get_param(chip, FG_DATA_BATT_ESR_REG, &batt_esr, false);
	if (rc) {
		pr_err("unable to read battery_esr: %d\n", rc);
		return rc;
	}

	rc = fg_get_param(chip, FG_SETTING_RS_TO_RSLOW_DISCHG,
			&rs_to_rslow_dischg, false);
	if (rc) {
		pr_err("unable to read rs to rslow dischg: %d\n", rc);
		return rc;
	}

	rc = fg_get_param(chip, FG_SETTING_RS_TO_RSLOW_CHG,
			&rs_to_rslow_chg, false);
	if (rc) {
	...
	rc = fg_set_sram_param(chip, FG_SETTING_RS_TO_RSLOW_CHG,
			(u8 *)&rs_to_rslow_chg, false);
	if (rc) {
		pr_err("unable to write rs_to_rslow_chg: %d\n", rc);
		return rc;
	}

	rc = fg_set_sram_param(chip, FG_SETTING_RS_TO_RSLOW_DISCHG,
			(u8 *)&rs_to_rslow_dischg, false);
	if (rc) {
...
static int fg_of_battery_profile_init(struct fg_chip *chip)
{
	...
	rc = fg_get_param(chip, FG_SETTING_PROFILE_INTEGRITY_REG,
			(int *)&reg, false);
	if (rc) {
	...
	rc = fg_sram_read(chip, chip->batt_info.batt_profile,
			BATT_PROFILE_OFFSET, len, 0, 0);
	if (rc) {
		pr_err("failed to read profile rc=%d\n", rc);
		return rc;
	}

	//TODO: write profile and restart if necessary

	if (chip->batt_info.rconn_mohm > 0) {
		rc = fg_update_batt_rslow_settings(chip);
		if (rc) {
			dev_err(chip->dev, "failed to update rslow\n");
			return rc;
		}
	}
	rc = fg_get_param(chip, FG_DATA_NOM_CAP,
			&chip->batt_info.nom_cap_uah, 0);
	if (rc) {
	...
	chip->batt_info.nom_cap_unbound = (chip->batt_info.nom_cap_uah > 5100000 ||
				chip->batt_info.nom_cap_uah < 1500000);

	if (chip->batt_info.nom_cap_unbound) {
...
static int fg_parse_of_param(struct fg_chip *chip, enum fg_sram_param_id id)
{
	...
	return fg_set_sram_param(chip, id, (u8 *)&temp, false);
}
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		error = fg_get_param(chip, FG_DATA_CURRENT, &val->intval, false);
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		error = fg_get_param(chip, FG_DATA_VOLTAGE, &val->intval, false);
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_OCV:
		error = fg_get_param(chip, FG_DATA_OCV, &val->intval, false);
		break;
	...
	case POWER_SUPPLY_PROP_CYCLE_COUNT:
		error = fg_get_param(chip, FG_DATA_CYCLE_COUNT, &val->intval, false);
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_MIN:
		val->intval = 3000000; /* single-cell li-ion low end */
		break;
	case POWER_SUPPLY_PROP_CHARGE_COUNTER:
		error = fg_get_param(chip, FG_DATA_CHARGE_COUNTER, &val->intval, false);
		break;
