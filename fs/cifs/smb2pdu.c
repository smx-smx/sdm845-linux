		 ...
		 struct create_posix_rsp *posix)
{
	...
	if (sid_len < 0) {
		cifs_dbg(VFS, "XXX bad owner sid in posix create response\n");
		return;
	}
	...
	sid_len = posix_info_sid_size(sid, end);
	if (sid_len < 0) {
		cifs_dbg(VFS, "XXX bad group sid in posix create response\n");
		return;
	}
	memcpy(&posix->group, sid, sid_len);

	cifs_dbg(VFS, "XXX nlink=%d mode=%o reparse_tag=%x\n",
